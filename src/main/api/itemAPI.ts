...
async function fetchItemFromArsha(
  bdoItemId: number,
  region: string
): Promise<ArshaAPIResponse> {
  ...
  return new Promise((resolve, reject) => {
        ...
        response.on("end", () => {
          try {
            if (response.statusCode === 404) {
              reject(
                new Error(
                  `Item not found in ${region} region - item may not be available in this market`
                )
              );
              return;
            }
            ...
            if (response.statusCode === 500) {
              reject(
                new Error(`${region} market API is temporarily unavailable`)
              );
              return;
            }
            ...
            if (response.statusCode !== 200) {
              reject(
                new Error(
                  `HTTP ${response.statusCode}: Failed to fetch item data from ${region} market`
                )
              );
              return;
            }
            ...
            reject(
              new Error(`Failed to parse response from ${region} market API`)
            );
          }
        });
      }
    ...
    request.on("error", (error) => {
      reject(
        new Error(
          `Network error connecting to ${region} market: ${error.message}`
        )
      );
    });
...
export const itemHandlers = {
  ...
  "items:create-from-api": async (
    ...
    try {
      ...
      const existingItem =
        await itemsService.getByBdoIdAndRegionIncludingArchived(
          bdoItemId,
          region
        );
      if (existingItem.success && existingItem.data) {
        ...
        if (existingItem.data.archived) {
          console.log(
            `Item ${bdoItemId} found archived for region ${region}, unarchiving...`
          );
          const unarchiveResult = await itemsService.unarchive(
          ...
          if (unarchiveResult.success) {
            ...
            return {
              success: false,
              error: `Failed to unarchive item: ${unarchiveResult.error}`,
            };
          }
        } else {
          // Item exists and is active
          console.log(
            `Item ${bdoItemId} already exists and is active for region ${region}`
          );
          return {
            success: true,
            skipped: true,
      ...
      const newItem = {
        ...
        base_price:
          conversionData?.type === "trash_loot" && conversionData?.base_price
            ? conversionData.base_price
            : arshaData.basePrice,
        ...
        convertible_to_bdo_item_id:
          conversionData?.convertible_to_bdo_item_id || null,
        conversion_ratio: conversionData?.conversion_ratio
          ? parseInt(conversionData.conversion_ratio)
          : 1,
      };
  ...
  "items:create-manual": async (
    ...
    try {
      ...
      const effectiveRegion =
        itemData.type === "trash_loot" || itemData.type === "conversion"
          ? null
          : itemData.region;
      ...
      const existingItem = effectiveRegion
        ? await itemsService.getByBdoIdAndRegionIncludingArchived(
            itemData.bdo_item_id,
            effectiveRegion
          )
        : await itemsService.getByBdoIdGlobalIncludingArchived(
            itemData.bdo_item_id
          );
      ...
      if (existingItem.success && existingItem.data) {
        ...
        if (existingItem.data.archived) {
          console.log(
            `Item ${itemData.bdo_item_id} found archived ${
              effectiveRegion ? `for region ${effectiveRegion}` : "globally"
            }, unarchiving...`
          );
          const unarchiveResult = await itemsService.unarchive(
            existingItem.data.id
          ...
          if (unarchiveResult.success) {
            return {
              ...
              message: `Item unarchived ${
                effectiveRegion ? `for ${effectiveRegion} region` : "globally"
              }`,
            };
          } else {
            return {
          ...
          // Item exists and is active
          console.log(
            `Item ${itemData.bdo_item_id} already exists and is active ${
              effectiveRegion ? `for region ${effectiveRegion}` : "globally"
            }`
          );
          return {
            ...
            message: `Item already exists ${
              effectiveRegion ? `in ${effectiveRegion} region` : "globally"
            }`,
          };
        }
      }
      ...
      const result = await itemsService.create(newItem);
      console.log(
        `Created manual item: ${itemData.name} ${
          effectiveRegion ? `for region ${effectiveRegion}` : "globally"
        }`
      );
      return result;
  ...
  "items:sync-prices": async (event: IpcMainInvokeEvent, region: string) => {
    try {
      ...
      if (!regionItemsResult.success || !regionItemsResult.data) {
        return {
          success: false,
          error: regionItemsResult.error || "Failed to get items for region",
        };
      }
      ...
      for (const item of marketplaceItems) {
        try {
          ...
          console.warn(
            `Failed to sync prices for item ${item.bdo_item_id} in ${region}:`,
            error
          );
        }
      }
    ...
    try {
      ...
      if (item.image_url) {
        ...
        if (!removeResult.success) {
          console.warn(
            "⚠️ [BACKEND] Failed to remove image from storage:",
            removeResult.error
          );
      ...
      const updateResult = await adminDatabase.updateItemAsAdmin(itemId, {
        image_url: null,
      });
  ...
  "items:upload-image-for-bdo-item": async (
    event: IpcMainInvokeEvent,
    bdoItemId: number,
    imageBuffer: Buffer,
    originalName: string
  ...
  "items:get-by-bdo-item-id": async (
    event: IpcMainInvokeEvent,
    bdoItemId: number
  ) => {
    try {
